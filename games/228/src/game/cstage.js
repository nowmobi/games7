...
cstage.inherit({
	...
	create:function(){
		...
		this.bg = game.add.sprite(this.centerX, this.centerY, 'bg_playscreen', null);
		...
		this.title = global.addText(0, -(this.titleBox.height * 0.09), _t('stageselect'), 50, game_config.font1);
		...
		this.achievBtn = new clickBtn(this.stageBox.x + (this.stageBox.width * 0.4), this.stageBox.y + (this.stageBox.height * 0.46), 'achievement');
		this.achievBtn.onClick.add(function(obj){
			obj.changePage('cachievementPage');
		}, this);
		this.gContent.add(this.achievBtn);
		...
		for(var a = 0; a < 3; a++){
			...
			var x =(-(page.width * 1.3)) + (a * page.width * 1.3);
			var y = this.stageBox.height * 0.4;
		...
		this.nextBtn = new clickBtn(this.pages[2].x + (this.pages[2].width * 1.8), this.prevBtn.y, 'nav_right');
		...
		this.closeBtn.onClick.add(function(obj){
			obj.changePage('cmainmenu');
		}, this);
		this.gButton.add(this.closeBtn);

		this.pageNow = 0;
		if(plRef.stageOpened.length < gameData.totalStage){
			this.pageNow = parseInt(plRef.stageOpened.length / this.stages.length);			
		} else {
			this.pageNow = 2;
		}
	...
	generateStages:function(){
		...
		for(var a = 0; a < 4; a++){
			for(var b = 0; b < 3; b++){
				var id = this.stages.length;
				var stage = new stageBtn(0, 0, id);
				var x = (-(stage.stageBtn.width * 1.8)) + (b * (stage.stageBtn.width * 1.8));
				var y = (-(this.stageBox.height * 0.5) + (stage.stageBtn.height * 0.9)) + (a * (stage.stageBtn.height * 1.5));
				stage.position.setTo(x, y);
				this.gContent.add(stage);
	...
	setPage:function(){
		...
		for(var a = 0; a < this.stages.length; a++){
			...
			var id = a + (this.pageNow * this.stages.length);
			if(id < gameData.totalStage){
				this.stages[a].visible = true;
				this.stages[a].id = id;
				this.stages[a].setStage();				
			}
		}
	},
	...
	update:function(){
		...
		var tempTotal = (this.pageNow + 1) * this.stages.length;
		if(tempTotal < gameData.totalStage){
